...
void bnode<T>::split(bnode *node, int index) {
    ...
    for (int j = 0; j < degree - 1; j++) {
        ...
        node1->datas[j] = node->datas[j + degree];
    }
...
void bnode<T>::borrow_key_before(int i) {
    ...
    datas[i - 1] = sib->datas[sib->key_numbers - 1];
